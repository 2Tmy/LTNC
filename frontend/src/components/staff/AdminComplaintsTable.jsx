          ...
          <tbody className="divide-y divide-slate-100">
            {complaints.map((complaint) => (
              <tr key={complaint.id} className="transition-colors hover:bg-slate-50">
                <td className="whitespace-nowrap px-lg py-md text-h3 text-primary">{complaint.id}</td>
                <td className="px-lg py-md">
                  <div className="flex items-center gap-sm">
                    <span className={`flex h-9 w-9 items-center justify-center rounded-full text-body-sm font-bold ${complaint.avatarClassName}`}>
                      {complaint.initials}
                    </span>
                    <span className="min-w-[110px] text-body-lg font-medium text-on-surface">{complaint.customer}</span>
                  </div>
                </td>
                <td className="min-w-[230px] px-lg py-md text-body-lg text-secondary">{complaint.title}</td>
                ...
                <td className="whitespace-nowrap px-lg py-md text-body-lg text-secondary">{complaint.department}</td>
